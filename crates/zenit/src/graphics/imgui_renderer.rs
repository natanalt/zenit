...
pub struct ImGuiTexture {
    ...
    wgpu_id: Option<wgpu::Id<Texture>>,
    bind_group: Option<wgpu::BindGroup>,
    handle: ImGuiTextureHandle,
}
...
impl ImGuiTexture {
    pub fn from_texture(handle: TextureHandle) -> Self {
        Self {
            wgpu_id: None,
            bind_group: None,
            handle: ImGuiTextureHandle::Texture(handle),
        }
    }
    ...
    pub fn from_camera(handle: CameraHandle) -> Self {
        Self {
            wgpu_id: None,
            bind_group: None,
            handle: ImGuiTextureHandle::Camera(handle),
        }
    }
    ...
    pub fn try_update_bind_group(&mut self, renderer: &mut Renderer, layout: &BindGroupLayout) {
        match &self.handle {
            ImGuiTextureHandle::Texture(handle) => {
                let texture = renderer.textures.get(handle);
                let texture_id = texture.handle.global_id();

                if Some(texture_id) != self.wgpu_id || self.bind_group.is_none() {
                    self.wgpu_id = Some(texture_id);
                    self.bind_group =
            ...
            ImGuiTextureHandle::Camera(handle) => {
...
pub struct ImguiRenderer {
    ...
    texture_handles: Vec<Arc<TextureId>>,
    cleanup_queue: Vec<TextureId>,
...
impl ImguiRenderer {
    ...
    pub fn new(r: &mut Renderer, format: wgpu::TextureFormat) -> Self {
        ...
        Self {
            ...
            texture_handles: Vec::with_capacity(10),
            cleanup_queue: Vec::with_capacity(10),
            pipeline,
            texture_bind_layout,
            uniform_bind_group,
            uniform_buffer,
    ...
    pub(in crate::graphics) fn setup_textures(
        &mut self,
        renderer: &mut Renderer,
        new_textures: impl Iterator<Item = (Arc<TextureId>, ImGuiTexture)>,
        new_font: Option<ImGuiTexture>,
    ) {
        // Add new textures
        for (id, texture) in new_textures {
        ...
        // Do garbage collection on abandoned handles
        self.texture_handles.retain_mut(|handle| {
            if let Some(id) = Arc::get_mut(handle) {
                // Enqueue the texture for deletion next frame
                // (We don't do this immediately, as an abandoned texture may still be queued for
                // rendering in the render dat)
                self.cleanup_queue.push(*id);
                false
            } else {
                true
            }
        ...
        // Update the font atlas if requested
        if let Some(new_font) = new_font {
            self.textures.insert(TextureId::new(0), new_font);
        }
